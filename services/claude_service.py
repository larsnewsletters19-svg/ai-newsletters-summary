...
class ClaudeService:
    ...
    def _generate_short_description(self, newsletters, videos, week_number):
        ...
        # Ta top 4-5 ämnen från newsletters
        topics_prompt = f"""Baserat på dessa newsletters, skapa en kort punktlista med 4-5 nyckelh ändelser denna vecka.

NEWSLETTERS:
{chr(10).join([f"- {nl['subject']}" for nl in newsletters[:10]])}
...
KRITISKT FORMAT för Teams Loop: 
- Mellan VARJE punkt måste finnas en HELT TOM rad
- Punkterna ska INTE vara på samma rad
- Se exempel nedan hur det MÅSTE se ut

Exempel på KORREKT format (observera tomma rader mellan punkter):

• **Första newsletter-titel** - [Läs här](länk)

• **Andra newsletter-titel** - [Läs här](länk)

• **Tredje newsletter-titel** - [Läs här](länk)

Lista ALLA newsletters från listan ovan i detta format.
