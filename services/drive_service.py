...
class DriveService:
    ...
    def save_newsletter(self, newsletter, folder_id):
        ...
        try:
            # Rensa och förbereda HTML för bättre PDF-rendering
            cleaned_html = self._prepare_html_for_pdf(newsletter['html_body'], newsletter['subject'])
            
            # Konvertera HTML till PDF med xhtml2pdf
            pdf_buffer = BytesIO()
            pisa_status = pisa.CreatePDF(
                BytesIO(cleaned_html.encode('utf-8')),
                dest=pdf_buffer,
                encoding='utf-8'
            )
            
            if pisa_status.err:
                print(f"PDF conversion warning for {safe_subject}: {pisa_status.err}")
    ...
    def _prepare_html_for_pdf(self, html_body, subject):
        """Rensa och förbered HTML för PDF-konvertering"""
        # Lägg till basic CSS för bättre rendering
        prepared_html = f"""
        <!DOCTYPE html>
        <html>
        <head>
